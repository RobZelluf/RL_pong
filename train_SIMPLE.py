...
import argparse
import wimblepong
from SIMPLE_SAA.SIMPLE_SAA import *
from utils import *
import pickle
import os
...
if args.save and not args.load:
    ...
    if not os.path.exists("SIMPLE_SAA/" + model_name):
        os.mkdir("SIMPLE_SAA/" + model_name)
...
if args.load:
    DIRs = [x for x in os.listdir("SIMPLE_SAA/") if os.path.isdir("SIMPLE_SAA/" + x) and "cache" not in x]
    i = 0
    for DIR in DIRs:
        print(i, DIR)
        i += 1
    ...
    with open("SIMPLE_SAA/" + model_name + "/model_info.p", "rb") as f:
    ...
    player = SIMPLE_SAA(env, player_id, model_info=model_info, fc1_size=fc1_size)
else:
    player = SIMPLE_SAA(env, player_id, size=args.size, fc1_size=args.fc1_size)
    start_episode = 0
...
for i in range(start_episode, episodes):
    ...
    if i % 100 == 0:
        ...
        if args.save:
            torch.save(player.policy_net, "SIMPLE_SAA/" + model_name + "/policy_net.pth")
            model_info = dict()
            ...
            with open("SIMPLE_SAA/" + model_name + "/model_info.p", "wb") as f:
                pickle.dump(model_info, f)

            print("Model", model_name,"saved!")

            with open("SIMPLE_SAA/" + model_name + "/performance.txt", "a") as f:
